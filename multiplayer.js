...
function endMultiplayerGame() {
  ...
  gameComplete.className = "game-complete"; // hilangkan .multiplayer supaya konsisten
  gameComplete.innerHTML = `
    <div class="complete-content">
      <h2>${winner.score >= 30 ? "🎉 Permainan Selesai! 🎉" : "Permainan Selesai!"}</h2>
      ...
      <div class="final-rankings">
        ...
        <div class="player-rankings">
          ${ranking.map((p, i) => `
            <div class="player-rank${p === winner ? " winner" : ""}">
              <span>${i + 1}. ${p.name}</span>
              <span>Skor: ${p.score}</span>
              <span>Benar: ${p.questionsAnswered}</span>
              <span>❤️${p.lives}</span>
            </div>
          `).join("")}
        </div>
